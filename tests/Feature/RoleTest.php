...
class RoleTest extends Base
{
    public function setUp() : void
    {
        ...
        $this->modelTranslationClass = RoleTranslation::class;
        ...
        $this->rawData = [
            ...
            'translations' => [
                [
                    'language_code' => 'ru',
                    'display_name' => 'Test role ru',
                    'description' => 'Test role desc ru',
                ],
                [
                    'language_code' => 'uz',
                    'display_name' => 'Test role uz',
                    'description' => 'Test role desc uz',
                ],
                [
                    'language_code' => 'uzc',
                    'display_name' => 'Test role uzc',
                    'description' => 'Test role desc uzc',
                ]
            ],
        ];
    ...
    // toggle da agar header token berilsa va form ma'lumotlari jo'natilsa va type = 1 bo'lsa, 201 qaytishi kerak
    public function test_can_toggle_create_item()
    {
        $role = $this->createModels()[0];
        $permission = $this->createPermissions()[0];
    ...
    // toggle da agar header token berilsa va form ma'lumotlari jo'natilsa va type = 0 bo'lsa, 200 qaytishi kerak
    public function test_can_toggle_delete_item()
    {
        $role = $this->createModels()[0];
        $permission = $this->createPermissions()[0];
    ...
    protected function createPermissions($count = 1){
        ...
        return Permission::factory($count)->create($params)->each(function ($model) {

            PermissionTranslation::factory()->create(['permission_id' => $model->id, 'language_code' => 'uz']);
            PermissionTranslation::factory()->create(['permission_id' => $model->id, 'language_code' => 'uzc']);
            PermissionTranslation::factory()->create(['permission_id' => $model->id, 'language_code' => 'ru']);
